...
<x-section :background="$background" :anchor="$anchor" :wrapperAttributes="$wrapper_attributes" class="testimonials {{ $classes }}">
    @if($title)
        ...
        <x-grid :cols="$columns" gap="lg">
            @foreach($testimonials as $testimonial)
                <x-card variant="filled" padding="lg" class="flex flex-col h-full">
                    {{-- Quote Icon --}}
                    <svg class="w-8 h-8 mb-4 text-content-brand" fill="currentColor" viewBox="0 0 24 24">
