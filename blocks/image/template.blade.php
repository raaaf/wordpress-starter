...
@if($image)
    <section class="{{ $classes }} relative flex items-center justify-between image"
        ...
        <div class="flex flex-col items-center justify-center max-w-6xl px-6 mx-auto md:px-8 group">
            <div class="relative overflow-hidden shadow-xl will-change-transform transition @if($showBorder) border-4 rounded-md border-{{ $bgColor }} @endif ease-in-out group lg:group-hover:shadow-2xl">
                @php
                    $size = 'full';
                ...
                @if ($alt && $showCaption)
                    <p id="img-desc-{{ $image }}"
                        ...
                        <span class="max-w-xs px-3 py-1 text-white border-2 rounded-md bg-primary/90 has-small-bold border-marine/20">
                            {{ $alt }}
                        </span>
                    </p>
                @endif
