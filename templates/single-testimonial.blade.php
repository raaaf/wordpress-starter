    ...
    <x-section background="primary">
        <article class="max-w-3xl mx-auto">
            ...
            <div class="bg-surface-secondary rounded-xl p-8 md:p-12">
                {{-- Quote Icon --}}
                <svg class="w-12 h-12 text-brand opacity-30 mb-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                ...
                @if($rating)
                    <div class="flex gap-1 mb-6" role="img" aria-label="{{ sprintf(__('%d von 5 Sternen', 'wp-starter'), $rating) }}">
                        @for($i = 1; $i <= 5; $i++)
                            <svg class="w-6 h-6 {{ $i <= $rating ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                ...
                <footer class="flex items-center gap-4">
                    ...
                    @else
                        <div class="w-16 h-16 rounded-full bg-surface-tertiary flex items-center justify-center">
                            <span class="text-2xl font-bold text-content-subtle">
                ...
                @if($sourceUrl)
                    <div class="mt-8 pt-6 border-t border-line">
                        <a
                            ...
                            class="inline-flex items-center gap-2 text-sm text-content-subtle hover:text-brand transition-colors"
                        >
            ...
            <div class="mt-8 text-center">
                <a href="{{ home_url() }}" class="inline-flex items-center gap-2 text-content-subtle hover:text-brand transition-colors">
