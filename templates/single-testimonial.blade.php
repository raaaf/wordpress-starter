...
@section('content')
    ...
    <x-section background="primary">
        <article class="max-w-3xl mx-auto">
            ...
            {{-- Testimonial Card --}}
            <div class="bg-surface-secondary surface-sheen border border-line rounded-[var(--card-radius)] p-8 md:p-12">
                {{-- Quote Icon --}}
                ...
                @if($rating)
                    <div class="flex gap-1 mb-6" role="img" aria-label="{{ sprintf(__('%d von 5 Sternen', 'wp-starter'), $rating) }}">
                        @for($i = 1; $i <= 5; $i++)
                            <svg class="w-6 h-6 {{ $i <= $rating ? 'text-icon-brand' : 'text-icon-disabled' }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
