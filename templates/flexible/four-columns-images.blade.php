{{--
    ...
    ACF Fields: show_section_header, section_chip, section_headline, section_description, image_1, accordion_1, image_2, accordion_2, image_3, accordion_3, image_4, accordion_4, background_color
--}}
            ...
            <x-card variant="outlined" padding="none" class="overflow-hidden">
                ...
                @if($lbl || $text)
                    <div class="p-6 lg:p-8 {{ !empty($acc) ? 'pb-0 lg:pb-0' : '' }}">
                        @if($lbl)
                            <p class="text-overline text-content-secondary mb-2">{{ $lbl }}</p>
                        @endif
                        @if($text)
