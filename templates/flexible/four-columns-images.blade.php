{{--
    Four Columns Images - Flexible Content Layout

    Uses shared components: x-section, x-grid, x-prose, x-card, x-section-header
    ACF Fields: show_section_header, section_chip, section_headline, section_description, image_1, column_1, image_2, column_2, image_3, column_3, image_4, column_4, background_color
--}}
...
<x-section :anchor="$sectionAnchor" :background="$background" class="four-columns-images">
    ...
    <x-grid cols="4" gap="xl">
        ...
        <x-card variant="outlined" padding="none" class="overflow-hidden">
            @if($image_4 && !empty($image_4['url']))
                <img src="{{ $image_4['url'] }}"
                     alt="{{ $image_4['alt'] ?? '' }}"
                     @if(!empty($image_4['width']) && !empty($image_4['height']))width="{{ $image_4['width'] }}" height="{{ $image_4['height'] }}"@endif
                     class="w-full object-cover"
                     loading="lazy">
            @endif
            <div class="p-6 lg:p-8">
