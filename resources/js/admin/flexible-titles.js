...
(function () {
  ...
  function getLayoutPreview(layout) {
    ...
    for (const fieldName of TITLE_FIELDS) {
      const field = layout.querySelector(
    ...
    for (const fieldName of CONTENT_FIELDS) {
      ...
      const wysiwygTextarea = layout.querySelector(
      ...
      const wysiwyg = layout.querySelector(`[data-name="${fieldName}"] iframe`);
      if (wysiwyg) {
        try {
  ...
  function updateLayoutTitle(layout) {
    ...
    const titleEl = layout.querySelector('.acf-fc-layout-title');
    if (!titleEl) {
      return;
    }
    ...
    let previewSpan = handle?.querySelector('.layout-preview-text');
   ...
   */
  function updateAllLayoutTitles() {
