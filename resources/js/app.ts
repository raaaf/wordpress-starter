...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const contentBlocks = document.querySelectorAll<HTMLAnchorElement>('.prose a, .one-column a, .two-columns a, .three-columns a, .four-columns a, .two-columns-images a, .one-third-columns a');
    ...
    contentBlocks.forEach(link => {
        ...
        const parentBlock = link.closest('[class*="-column"], .hero, .cta-block, .video, .accordion, .two-columns-images, .one-third-columns');
        if (parentBlock) {
            const blockType = parentBlock.className.match(/(one|two|three|four)-column(?:s)?(?:-images)?|one-third-columns|hero|cta-block|video|accordion/)?.[0] || 'unknown';
